...
export class FormItemRadioComponent implements FormItemWidget, OnInit {
    ...
    ngOnInit() {
        ...
        let selectedOptionObj = this.item.children.find(op => op.optionValue === this.item.value);
        ...
        if (!this.selectedOption && this.item.value && !this.item.multiple) {
            ...
            selectedOptionObj = this.item.children.find(op => op.optionValue === valArr[0]);
    ...
    onExplanationValueChanges(val: string, selectedOptionVal: string | string[]) {
        ...
        const option = this.item.children.find(op => op.optionValue === selectedOptionVal);
