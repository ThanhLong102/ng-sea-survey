...
describe('FormItemRadioComponent', () => {
    ...
    it('should render '+item.children.length+' list items', () => {
        const compiled = fixture.debugElement.nativeElement;
        const options = [...compiled.querySelector('mat-selection-list').children];
        expect(options.length).toEqual(item.children.length);
        options.forEach(option=>{
            expect(option.querySelector('.mat-list-text').textContent.trim()).toEqual(item.children[options.indexOf(option)].title);
        })
    });
    ...
    it('should render active list option in accordance field value', () => {
        ...
        item.value=item.children[1].optionValue;
        fixture.detectChanges();
    ...
    it('should render active list with MULTIPLE option selected in accordance field value', () => {
        ...
        expect(options.filter(option=>option.getAttribute('aria-selected')==="true").length).toEqual(0);
        item.multiple=true;
        item.value=[item.children[1].optionValue, item.children[3].optionValue];
        fixture.detectChanges();
        expect(options[1].getAttribute('aria-selected')).toEqual('true');
    ...
    it('should change the field value by click on list item', () => {
        ...
        expect(item.value).toEqual(item.children[0].optionValue);

        options[1].click();
        fixture.detectChanges();

        expect(item.value).toEqual(item.children[1].optionValue);
    });
    ...
    it('should change the field value by click on list items in MULTIPLE options mode', () => {
        ...
        expect(item.value).toEqual([item.children[0].optionValue, item.children[2].optionValue]);

        options[0].click();
        fixture.detectChanges();

        expect(item.value).toEqual([item.children[2].optionValue]);

        options[1].click();
        fixture.detectChanges();

        expect(item.value).toEqual([item.children[2].optionValue, item.children[1].optionValue]);

        options[3].click();
        fixture.detectChanges();

        expect(item.value).toEqual([item.children[2].optionValue, item.children[1].optionValue, item.children[3].optionValue]);
    });
